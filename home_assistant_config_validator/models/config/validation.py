...
from jinja2.defaults import VARIABLE_START_STRING, BLOCK_START_STRING, COMMENT_START_STRING
...
from home_assistant_config_validator.utils import InvalidTemplateVarsError
...
class ValidationConfig(Config):
    ...
    def _validate_jinja2_templates(self, entity: Entity, /) -> None:
        ...
        def _cb(value: str, dict_key: str | None = None, **_:Any) -> str:
            if VARIABLE_START_STRING not in value and  BLOCK_START_STRING not in value and COMMENT_START_STRING not in  value:
                return value
            ...
            if undeclared_variables:
                ...
                def _remove_known_variables(value:dict, dict_key: str | None = None, **_:Any) -> dict:
                    nonlocal undeclared_variables
                    if dict_key == "variables":
